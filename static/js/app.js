/**
 * Agenda de Trabalho - JavaScript Principal
 */
...
function inicializarCalendario() {
    ...
    calendar.className = `grid grid-cols-${diasExibidos.length} gap-2`;
    
    // Adicionar colunas dos dias
    diasExibidos.forEach(({ dia, nome }) => {
        const dayElement = document.createElement('div');
        dayElement.className = 'border p-2 relative';
...
function renderizarCompromissos() {
    ...
    compromissos.forEach(compromisso => {
        ...
        appointmentElement.className = 'absolute w-[95%] rounded text-white p-2 cursor-pointer transition-all hover:shadow-lg';
        ...
        appointmentElement.innerHTML = `
            <div class="flex justify-between items-center">
                <strong>${local.nome} (${compromisso.duracao} ${compromisso.tipo_hora})</strong>
                <div>
